...
link:#BNBUH[Table 40-1] describes the BNF symbols used in this chapter.
...
link:#BNBVD[Table 40-2] lists the query language operators in order of
...
characters in the pattern value. link:#BNBVH[Table 40-3] shows some
...
of a query. The functions are listed in link:#BNBVP[Table 40-4],
link:#BNBVQ[Table 40-5], and link:#GJJNL[Table 40-6].

In link:#BNBVP[Table 40-4], the `start` and `length` arguments are of
...
In link:#BNBVQ[Table 40-5], the `number` argument can be an `int`, a
`float`, or a `double`.
...
In link:#GJJNL[Table 40-6], the date/time functions return the date,
...
defined by link:#BNBVS[Table 40-7] and link:#BNBVT[Table 40-8]. (In
...
primary keys have the same value. link:#BNBVV[Table 40-9] shows the
operator logic of a negation, and link:#BNBVW[Table 40-10] shows the
